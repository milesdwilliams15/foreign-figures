...
dt |>
  mutate(
    ...
    usa_influence = ifelse(
      american_inf_1 %in% c("Some", "A lot"), 1, 0
    ),
    chn_influence = ifelse(
      chinese_inf_1 %in% c("Some", "A lot"), 1, 0
    ),
...
bind_rows(
  ...
  labs(
    title = str_wrap(
      'Share saying that American and China have "some" or "a lot" of influence',
      width = 37
    ),
    subtitle = str_wrap(
      "Source: Allen, Flynn, Machain, & Stravers (2020) APSR.",
...
```{r}
lm(
  chinese_gov_rec ~ american_gov_rec, dt
) |> summary()
```
