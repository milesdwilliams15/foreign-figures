...
dt |>
  ...
  ggplot() +
  aes(diff, as.factor(atop_defense)) +
  stat_summary(fun.data = ~ my_mean_ci(.x, .84), color = "navy") +
  geom_vline(
    xintercept = 0,
    lty = 2
  ) +
  labs(
    title = str_wrap(
      "Cuts in 2025 obligations to allies weren't much deeper than to non-allies",
      width = 52
    ),
    subtitle = str_wrap(
      "% change in obligations with bootstrapped 84% confidence intervals"
    ),
    y = NULL,
    x = NULL,
    caption = logo
