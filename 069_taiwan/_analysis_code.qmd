...
dt |>
  group_by(year, tw) |>
  summarize(
    across(ends_with("_amt"), ~ mean(.x, na.rm = T))
  ) |>
  ungroup() |>
  mutate(
    tw = ifelse(tw == 1, "Yes", "No")
  ) |>
  pivot_longer(
    3:5
  ) |>
  ...
  labs(
    title = str_wrap(
      "The best way to access Chinese development finance is ending Taiwanese recognition",
      width = 40
    ),
    subtitle = "Average financing in billions of 2021 USD",
    x = NULL,
    y = NULL,
    color = "Recognize Taiwan?",
    caption = logo
  ) +
  ggpal("binary") +
  scale_y_continuous(
